...
// to avoid mallocating huge blocks (NOTE: is this necessary?)
typedef struct{
    dissimpair **pairs;
    int n_pairs;
    int n_partitions;
...
void *reduce_vr_thread_execution(void *arg){
...
void reduce_vr(const problem *prob,
    ...
    int *next_sols = safe_malloc(sizeof(int)*vision_range);
        ...
        for(int i=0;i<THREADS;i++){
            ...
            //
            int rc = pthread_create(&threads[i],NULL,reduce_vr_thread_execution,&targs[i]);
            if(rc){
